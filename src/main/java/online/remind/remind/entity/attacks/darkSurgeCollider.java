...
public class darkSurgeCollider extends ThrowableProjectile {
    ...
    public darkSurgeCollider(EntityType<? extends ThrowableProjectile> type, Level level){
        super(type, level);
        this.noPhysics = true;
    ...
    public darkSurgeCollider(Level level, LivingEntity caster, float damage){
        ...
        for (LivingEntity target : entities) {
            if (target != getOwner()) {
                ...
                if (p == null || (p.getMember(target.getUUID()) == null || p.getFriendlyFire())) {
                    ...
                    target.hurt(KKDamageTypes.getElementalDamage(KKDamageTypes.DARKNESS, this, this.getOwner()), damage);
