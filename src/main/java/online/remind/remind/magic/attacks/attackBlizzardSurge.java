...
public class attackBlizzardSurge extends Magic {
    ...
    public attackBlizzardSurge(ResourceLocation registryName, boolean hasToSelect, int tier, ResourceLocation gmAbility) {
...
setTier(tier);
    ...
    public void magicUse(LivingEntity player, LivingEntity caster, float fullMPBlastMult, LivingEntity lockOnEntity) {
        ...
        switch(getTier()){
            case 0:
                dmg = casterStrengthStat(caster) * (abilityStacks(caster, ModAbilities.BLIZZARD_BOOST) * 0.1f);
                break;
            case 1:
                dmg = (casterStrengthStat(caster) * 1.1f) * (abilityStacks(caster, ModAbilities.BLIZZARD_BOOST) * 0.1f);
                break;
            case 2:
                dmg = (casterStrengthStat(caster) * 1.25f) * (abilityStacks(caster, ModAbilities.BLIZZARD_BOOST) * 0.1f);
                break;
        }
